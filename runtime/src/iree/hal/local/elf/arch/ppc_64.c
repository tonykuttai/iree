...
#if defined(IREE_ARCH_PPC_64)

// Documentation:
// https://refspecs.linuxfoundation.org/ELF/ppc64/PPC-elf64abi.html
// https://github.com/power-devops/powerpc-abi/blob/master/ELF_ABI.md
...
bool iree_elf_machine_is_valid(iree_elf_half_t machine) {
  return machine == 0x15;  // EM_PPC64 / 21
}
...
enum {
  IREE_ELF_R_PPC64_NONE = 0,
  IREE_ELF_R_PPC64_ADDR64 = 38,
  IREE_ELF_R_PPC64_GLOB_DAT = 20,
  IREE_ELF_R_PPC64_JMP_SLOT = 21,
  IREE_ELF_R_PPC64_RELATIVE = 22,
};

static iree_status_t iree_elf_arch_ppc64_apply_rela(
  ...
  for (iree_host_size_t i = 0; i < rela_count; ++i) {
    ...
    switch (type) {
      case IREE_ELF_R_PPC64_NONE:
        break;
      case IREE_ELF_R_PPC64_ADDR64:
        *(uint64_t*)instr_ptr = (uint64_t)(sym_addr + rela->r_addend);
        break;
      case IREE_ELF_R_PPC64_GLOB_DAT:
      case IREE_ELF_R_PPC64_JMP_SLOT:
        *(uint64_t*)instr_ptr = (uint64_t)(sym_addr + rela->r_addend);
        break;
      case IREE_ELF_R_PPC64_RELATIVE:
        *(uint64_t*)instr_ptr = (uint64_t)(state->vaddr_bias + rela->r_addend);
        break;
      default:
        return iree_make_status(IREE_STATUS_UNIMPLEMENTED,
                                "unimplemented ppc64 relocation type %08X",
                                type);
    }
  }
...
iree_status_t iree_elf_arch_apply_relocations(
  ...
  if (rela_count > 0) {
    IREE_RETURN_IF_ERROR(
        iree_elf_arch_ppc64_apply_rela(state, rela_count, rela_table));
  }
